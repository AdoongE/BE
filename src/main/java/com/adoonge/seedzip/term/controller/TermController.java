...
public class TermController {
    ...
    @PatchMapping("/{type}")
    @Operation(summary = "약관 수정 API", description = "약관 수정 API입니다.")
    public ApiResponse<TermResponse> updateTerm(@PathVariable TermType type,
                                                @RequestBody TermUpdateRequest request,
                                                @AuthenticationPrincipal CustomUserDetails customUserDetails
    ) {
        termService.updateTerm(type, request, customUserDetails.getMember());
