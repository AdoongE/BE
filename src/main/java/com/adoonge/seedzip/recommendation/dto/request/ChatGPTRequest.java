...
public class ChatGPTRequest {
    ...
    public static ChatGPTRequest createImageRequest(String model, int maxTokens, String imageUrl) {
        Message systemMessage = new TextMessage("system", "You are a helpful assistant for generating hashtags for image. Please answer in Korean.");
    ...
    public static ChatGPTRequest createPdfRequest(String model, int maxTokens, String requestText) {
        Message systemMessage = new TextMessage("system", "You are a helpful assistant for generating hashtags for Pdf. Please answer in Korean.");
        Message userMessage = new TextMessage("user", "Analyze the given text about a pdf, suggest a title, provide a 3-sentence summary, and 3 tags as a comma-separated string without #. Return the result as a raw JSON object without any code block or additional formatting.\n" + requestText);
        return createChatGPTRequest(model, maxTokens, List.of(systemMessage,userMessage));
    }
